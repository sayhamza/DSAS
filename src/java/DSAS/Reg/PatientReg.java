...
public class PatientReg extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (count > 0) {
                response.sendRedirect("Patients.jsp?mailid");
            } else {

                try {
                    int i = st.executeUpdate("insert into patients(name,email,dob,phone,address, pass ,ustatus, regtime,vparam,pid)values('" + name + "','" + mail + "','" + dob + "','" + phone + "','" + address + "','" + pass + "','No','" + time + "','No','"+pid+"') ");
                    if (i != 0) {
                        System.out.println("success");
                        response.sendRedirect("Patients.jsp?success");
                    } else {
                        System.out.println("Patients.jsp?failed");
                    }
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
